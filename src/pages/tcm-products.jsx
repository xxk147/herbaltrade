...
// @ts-ignore;
import { Search, Filter as FilterIcon, ChevronLeft, ChevronRight, ShoppingCart, Heart, Star, Leaf, Menu, X, Globe, Phone, Mail, MapPin, Send, Facebook, Twitter, Instagram, Linkedin } from 'lucide-react';
// @ts-ignore;
import { Input, Button, Card, CardContent, CardFooter, Badge, Textarea } from '@/components/ui';
...
function TCMHeader({
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm border-b border-green-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex space-x-8">
            {menuItems.map(item => <a key={item.name} href={item.href} className="text-green-700 hover:text-green-900 px-3 py-2 text-sm font-medium transition-colors">
                {item.name}
              </a>)}
          </nav>

          <div className="hidden md:flex items-center space-x-4">
            <select value={currentLang} onChange={e => handleLanguageChange(e.target.value)} className="w-20 bg-green-50 border border-green-200 rounded px-2 py-1 text-sm">
              {languages.map(lang => <option key={lang.code} value={lang.code}>{lang.code}</option>)}
            </select>
          </div>
        ...
        {isMenuOpen && <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {menuItems.map(item => <a key={item.name} href={item.href} className="text-green-700 hover:text-green-900 block px-3 py-2 text-base font-medium" onClick={() => setIsMenuOpen(false)}>
                  {item.name}
                </a>)}
            </div>
          </div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">人参系列</a></li>
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">当归系列</a></li>
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">黄芪系列</a></li>
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">枸杞系列</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">产品目录</a></li>
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">质量认证</a></li>
              <li><a href="#" className="text-green-200 hover:text-white transition-colors">出口服务</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              <li className="text-green-200">电话: +86 558 5555 8888</li>
              <li className="text-green-200">邮箱: info@herbaltrade.com</li>
              <li className="text-green-200">地址: 安徽省亳州市谯城区</li>
            </ul>
          </div>
        </div>
...
export default function TCMProductsPage(props) {
  ...
  return <div style={props.style} className="min-h-screen bg-gray-50">
      <TCMHeader onLanguageChange={handleLanguageChange} />
